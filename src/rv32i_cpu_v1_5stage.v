...
module rv32i_cpu #(
    ...
    input                timer_irq,    // timer interrupt (from CLINT/timer)
    input                soft_irq,     // software interrupt (from CLINT)

    // Debug interface
    input                debug_halt,    // halt request from debug module
    input                debug_resume,  // resume request from debug module
    output               debug_halted,  // CPU is halted
    output               debug_running, // CPU is running
    output        [31:0] debug_pc,      // current PC for debug read
    input          [4:0] debug_gpr_addr,  // GPR address for debug read/write
    input         [31:0] debug_gpr_wdata, // GPR write data
    output        [31:0] debug_gpr_rdata, // GPR read data
    input                debug_gpr_wr     // GPR write enable
);
    ...
    assign imem_req  = !(mem_stall || mul_div_stall || load_use_hazard || debug_stall);
    ...
    always @(posedge clk or negedge rst_n) begin
        ...
        end else if (!mem_stall && !mul_div_stall && !debug_stall) begin
    ...
    assign pc_stall = load_use_hazard || mem_stall || fetch_stall || mul_div_stall || debug_stall;

    // ID/EX register update
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n || (pipeline_flush && !mem_stall && !mul_div_stall && !debug_stall) || (load_use_hazard && !mem_stall && !mul_div_stall && !debug_stall)) begin
            idex_pc         <= 0;
            idex_rs1_data   <= 0;
            idex_rs2_data   <= 0;
            idex_imm        <= 0;
    ...
    // External interrupt pending — mip reflects hardware pins
    wire [31:0] mip_hw = {20'b0, ext_irq, 3'b0, timer_irq, 3'b0, soft_irq, 3'b0};
    wire [31:0] irq_pending = mip_hw & csr_mie & {32{csr_mstatus[3]}};  // MIE gate
    wire        irq_taken = |irq_pending && idex_valid && !ex_trap && !ex_mret;

    // Priority encode: ext > timer > sw (RISC-V spec)
    wire [31:0] irq_cause = irq_pending[11] ? 32'h8000_000B :  // MEIP
                            irq_pending[7]  ? 32'h8000_0007 :  // MTIP
                            irq_pending[3]  ? 32'h8000_0003 :  // MSIP
                                              32'h8000_000B;    // default MEIP
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // MIP reflects hardware interrupt pins (read-only bits)
            csr_mip <= mip_hw;
            ...
            if (memwb_valid && !mem_stall && !mul_div_stall && !debug_stall)
                csr_minstret <= csr_minstret + 64'd1;

            if (!debug_stall) begin
            // Hardware interrupt (higher priority than sync exceptions)
            if (irq_taken) begin
                csr_mepc    <= idex_pc;
    ...
    assign debug_pc      = ifid_pc;

    // Debug GPR access (reads register file, writes when halted)
    assign debug_gpr_rdata = (debug_gpr_addr == 5'd0) ? 32'd0 : regfile[debug_gpr_addr];

    wire debug_stall = debug_halted_r;
    ...
    assign pipeline_flush = irq_taken || ex_mispredict || ex_unpredicted_redirect;

    // Branch target: correction address for mispredictions, or normal redirect
    assign branch_target = irq_taken ? trap_target :
                           ex_trap   ? trap_target :
                           ex_mret   ? mret_target :
    ...
    wire ex_is_branch_or_jal = idex_valid && (idex_branch || idex_jal) &&
                               !mem_stall && !mul_div_stall && !debug_stall;
